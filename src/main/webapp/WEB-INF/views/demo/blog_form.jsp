...
<body>
	<form:form id="input-form" modelAttribute="blog"
		...
		<fieldset class="prepend-top">
			...
			<legend><fmt:message key="demo.gd.blog" /></legend>

			<div id="messageBox" class="error-msg" style="display: none"><fmt:message key="global.valid.errmsg" /></div>

			<div>
				<label for="title" class="field"><fmt:message key="blog.gd.col.title" />:</label> <input type="text"
					id="title" name="title" size="20" value="${blog.title}"
					class="required" />
			</div>
			<div>
				<label for="author" class="field"><fmt:message key="blog.gd.col.author" />:</label> <input type="text"
					id="author" name="author" size="20" value="${blog.author}"
					class="required" />
			</div>
			<div>
				<label for="category" class="field"><fmt:message key="blog.gd.col.category" />:</label> <input type="text"
					id="category" name="category" size="20" value="${blog.category}"
					class="required" />
			</div>
			<div>
				<label for="url" class="field"><fmt:message key="blog.gd.col.url" />:</label> <input type="text"
					id="url" name="url" size="20" value="${blog.url}" class="required" />
			</div>
			<div>
				<label for="other" class="field"><fmt:message key="blog.gd.col.other" />:</label> <input type="text"
					id="other" name="other" size="20" value="${blog.other}"
					class="required" />
			</div>
			<div>
				<label for="publishDate" class="field"><fmt:message key="blog.gd.col.publishDate" />:</label> <input
